# R/Rook template for OpenFaaS with of-watchdog

> Sends and receives JSON using [Rook](https://github.com/jeffreyhorner/Rook)
...
```bash
faas-cli new --lang rstats-base-rook r-base-rook-hello --prefix=dockeruser
```

## Customizing your function

Now we have a `r-base-rook-hello.yml` file and function folder `./r-base-rook-hello`.
...
Customize the `./r-base-rook-hello/handler.R` file as needed:
...
```bash
faas-cli up -f r-base-rook-hello.yml
```
...
```bash
docker run -p 4000:8080 dockeruser/r-base-rook-hello
```
...
```bash
faas-cli new --lang rstats-base-rook r-base-pca --prefix=dockeruser
```
