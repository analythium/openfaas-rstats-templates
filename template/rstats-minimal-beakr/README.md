...
```bash
faas-cli new --lang rstats-minimal-beakr r-minimal-beakr-hello --prefix=dockeruser
```

## Customizing your function

Now we have a `r-minimal-beakr-hello.yml` file and function folder `./r-minimal-beakr-hello`.
...
Customize the `./r-minimal-beakr-hello/handler.R` file as needed:
...
```bash
faas-cli up -f r-minimal-beakr-hello.yml
```
...
```bash
docker run -p 4000:8080 dockeruser/r-minimal-beakr-hello
```
...
```bash
faas-cli new --lang rstats-minimal-beakr r-minimal-pca --prefix=dockeruser
```
