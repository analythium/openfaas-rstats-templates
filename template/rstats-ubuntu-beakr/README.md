...
```bash
faas-cli new --lang rstats-ubuntu-beakr r-ubuntu-beakr-hello --prefix=dockeruser
```

## Customizing your function

Now we have a `r-ubuntu-beakr-hello.yml` file and function folder `./r-ubuntu-beakr-hello`.
...
Customize the `./r-ubuntu-beakr-hello/handler.R` file as needed:
...
```bash
faas-cli up -f r-ubuntu-beakr-hello.yml
```
...
```bash
docker run -p 4000:8080 dockeruser/r-ubuntu-beakr-hello
```
...
```bash
faas-cli new --lang rstats-ubuntu-beakr r-ubuntu-pca --prefix=dockeruser
```
